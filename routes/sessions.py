...
def chat(session_id):
    ...
    code = data.get('code', '')
    language = data.get('language', session.get('language', 'python'))
...
def _run_tests_for_session(session, user_code, client, language='python'):
    if language != 'python':
        return {
            ...
            'results': [],
            'error': (
                f'Automated interview test execution currently supports Python only. '
                f'Use Run for {language}, or switch back to Python for generated and pre-canned interview tests.'
            ),
        }
