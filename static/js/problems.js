...
const GENERATED_PROBLEMS_SESSION_KEY = 'generated_problems_tab';

function loadGeneratedProblemsFromSession() {
  try {
    const raw = sessionStorage.getItem(GENERATED_PROBLEMS_SESSION_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed.filter(p => p && p._ephemeral) : [];
  } catch (e) {
    sessionStorage.removeItem(GENERATED_PROBLEMS_SESSION_KEY);
    return [];
  }
}

function saveGeneratedProblemsToSession() {
  try {
    const generated = allProblems.filter(p => p && p._ephemeral);
    sessionStorage.setItem(GENERATED_PROBLEMS_SESSION_KEY, JSON.stringify(generated));
  } catch (e) { }
}

function mergeProblemsWithGenerated(baseProblems, generatedProblems) {
  const merged = new Map();
  [...baseProblems, ...generatedProblems].forEach(problem => {
    if (problem && problem.id !== undefined && problem.id !== null) {
...
async function loadProblems() {
  ...
  const persistedGenerated = loadGeneratedProblemsFromSession();
  allProblems = mergeProblemsWithGenerated(await res.json(), persistedGenerated);
  saveGeneratedProblemsToSession();
  renderProblems();
  updateProgressChip();
}
...
async function generateProblems() {
  ...
  try {
    ...
    if (generated.length > 0) {
      ...
      const taggedGenerated = generated.map(p => ({ ...p, _ephemeral: true }));
      allProblems = mergeProblemsWithGenerated(allProblems, taggedGenerated);
      saveGeneratedProblemsToSession();
      status.innerHTML = `Generated ${generated.length} new problem${generated.length > 1 ? 's' : ''}! (kept in this tab until you close it)<br>` +
        generated.map(p => `• <strong>${p.title}</strong> (${p.difficulty})`).join('<br>');
      renderProblems();
      updateProgressChip();
    }
